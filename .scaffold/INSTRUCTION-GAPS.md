...
- resilience.md says hedging must replace the standard handler (`RemoveAllResilienceHandlers()`); a GET-guarded hedging strategy nested inside the standard handler (D-051) is also valid and keeps retries for the hedged pair.
- `DisableForUnsafeHttpMethods()` guidance does not mention gRPC: every gRPC call is an HTTP POST, so a read-only gRPC client must keep retries explicitly (D-063).
- data-persistence.md Set-Based Writes limits `ExecuteUpdateAsync`/`ExecuteDeleteAsync` to non-aggregate tables; compare-and-set stamps and retention purges on aggregate tables with the predicate re-asserted in the WHERE and side effects staged in the same transaction are legitimate (TaskItemSystemRepository).
- grpc.md prescribes `EnableCallContextPropagation()` for every client; it applies only when the caller is itself serving a gRPC call. A non-gRPC caller (Blazor Server) sets an explicit per-call deadline instead.
...
- resource-implementation schema: database enum is [AzureSQL, SQLServer] only; a dual-provider reference cannot declare PostgreSQL (needs PostgreSQL and a multi-provider shape, e.g. databaseProviders list). Addressed for the enum value by scaffold-ai PR #2 (PostgreSQL added); a multi-provider shape is still open.
- resource-implementation schema: entities[].dataStore enum [sql, cosmosdb, table, blob] cannot mark a relational read model, a relational audit sink or a PostgreSQL-only table; aiServices.search.provider is a scalar enum [AzureAISearch, None] with no PgVector or Sql value; messagingProviders[].type enum [AzureServiceBus, EventGrid, EventHubs] has no RabbitMQ value (the reference declares one). RabbitMQ added by scaffold-ai PR #2.
- No guidance for the "package supersedes app-local fallback" step of a package-version-bump refactor: when a previously `// fallback:`-marked app-local type ships in the package, the instruction set does not say to delete the fallback and retarget every artifact line (UBIQUITOUS-LANGUAGE.md, resource-implementation.yaml, DESIGN-DECISIONS.md) that names it in the same commit; each of E1-E5 in this refactor had to work that out ad hoc.
- No guidance for rejecting a shipped package member on its merits: this refactor accepted `EF.Audit.Contracts` (interface only) but rejected `EF.Audit.Data`/`EF.Audit.AzureTable` (both reintroduce the consumer-clock timestamp defect the app had already fixed); the instruction set has no recipe for "the package shipped, but its default policy is wrong for this app, keep the app-side override and document why" as distinct from "the package has not shipped yet".
