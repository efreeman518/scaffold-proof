...
| Project | Category filter | Verified count | Notes |
|---|---|---:|---|
| Test.Integration | `TestCategory=Integration` | 59 (SqlServer) / 61 (PostgreSql) | Includes the new `RelationalTaskViewRepositoryTests`, `RelationalAuditLogRepositoryTests`, `S3ObjectStorageRepositoryTests` (MinIO Testcontainers), `RedisDistributedLockTests`, `MessagePackCacheTests`, and (PostgreSql only) `PgVectorSearchTests`. The 3-skipped-on-SqlServer / 1-skipped-on-PostgreSql split is deliberate provider-split assertions (e.g. pgvector tests skip on SqlServer), not flakiness |
...
7. Watch items recorded by `docs/plans/scale-guidance-alignment.md` and the G3/P6 session log, not yet addressed: `Pgvector.EntityFrameworkCore` 0.3.0 targets `net8.0`/`Npgsql >= 9.0.1` (rolls forward onto 10.0.3); floating image tags in `deploy/compose/` (`grafana/otel-lgtm:latest`, `minio/minio:latest`, `edoburu/pgbouncer:latest`); `launchSettings.json`'s `applicationUrl` conflicts with `Kestrel:Endpoints` on a local `dotnet run` (one override warning); `TaskFlow.Functions.csproj` still carries `OpenAI`/`Microsoft.Extensions.AI.OpenAI`/`Microsoft.AI.Foundry.Local` as `PrivateAssets="all"`; App Configuration is not wired into the Functions host; a second consecutive solution build emits ~180 `ExtensionsMetadataGenerator` warnings from a stale `TaskFlow.Functions` `obj` tree (clean-`obj` build is 0 warnings; not reproduced in this pass since this task did not need a second consecutive build).
