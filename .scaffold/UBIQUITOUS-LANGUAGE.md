...
| `Lane` | concept | One of the two hosting contracts, Azure or NonAzure, which owns core provider defaults and shared host/deployment behavior. | `TASKFLOW_LANE=Azure\|NonAzure`; `Portable` is a deprecated one-release input alias for NonAzure. See D-060. |
| `Azure lane` | concept | Hosting lane owning SQL Server, Service Bus, Azure Blob, Cosmos, Azure Table, and Blob Data Protection. | `TASKFLOW_LANE=Azure`. See D-060. |
| `NonAzure lane` | concept | Hosting lane owning PostgreSQL, RabbitMQ, S3, relational audit, PostgreSQL JSONB read model, and Redis Data Protection. | `TASKFLOW_LANE=NonAzure`; MongoDB is an explicit read-model opt-in. See D-060. |
| `Portable lane` | deprecated alias | Former name for the NonAzure lane, accepted as an input alias for one release only. | `TASKFLOW_LANE=Portable` normalizes to NonAzure. See D-035, D-060. |
| `Provider switch` | concept | One independent, provider-neutral seam selecting a concrete implementation of a port; lane-owned core conflicts fail fast. | Env/config override is allowed only when compatible with the selected lane; unknown or conflicting values fail fast. See D-034, D-060. |
| `Lane contract` | concept | The shared provider and host contract consumed by Aspire, Testcontainers, Azure Bicep, Compose, and React, Blazor, and Uno. | Expensive full-lane and UI acceptance is manual. See D-060. |
| `Read-model provider` | concept | The backing store for the denormalized `TaskView` projection: Cosmos in Azure, PostgreSQL JSONB by default in NonAzure, or explicitly selected MongoDB. | `ReadModel:Provider = Cosmos \| PostgreSqlJsonb \| MongoDb`; legacy `Relational` aliases `PostgreSqlJsonb` for one release. See D-038, D-060. |
| `Object-storage provider` | concept | The backing store for attachment binary content behind `EF.Storage.Contracts.IObjectStorageRepository`: Azure Blob or S3-compatible. | `Storage:Provider = AzureBlob \| S3`. See D-037. |
| `Audit sink` | concept | The backing store for `AuditLog` rows: Azure Table in Azure or relational audit in NonAzure. | `Audit:Provider = AzureTable \| Relational`. See D-039, D-060. |
