...
| D-036 | Hosting | Portable compute topology | Superseded by D-060. Historical decision retained: Docker Compose on one VPS with Caddy as the TLS edge. | D-035 | superseded | Compose remains the NonAzure deployment shape; the lane contract now covers Aspire, Testcontainers, Bicep, Compose, and all UIs. | P6 |
| D-037 | Storage | Object-storage provider switch | Superseded by D-060. Historical decision retained: `Storage:Provider = AzureBlob \| S3` behind the provider-neutral repository. | D-035, D-030 | superseded | NonAzure uses S3; MinIO is replaced by SeaweedFS. | P2, P4 |
| D-038 | Data | Read-model provider switch | Superseded by D-060. Historical decision retained: `ReadModel:Provider = Cosmos \| Relational` selected the read model arm. | D-035, D-004, D-030 | superseded | NonAzure now defaults to PostgreSQL JSONB (`PostgreSqlJsonb`), permits explicit MongoDB, and accepts legacy `Relational` as a one-release alias. | P2, P3 |
| D-039 | Data | Audit-sink provider switch | Superseded by D-060. Historical decision retained: `Audit:Provider = AzureTable \| Relational`. | D-035, D-030 | superseded | Azure uses Azure Table; NonAzure uses relational audit. | P2, P3 |
| D-040 | Search | Search provider switch | Superseded by D-060. Historical decision retained: `Search:Provider = AzureAiSearch \| PgVector \| Sql`, with PostgreSQL required for `PgVector`. | D-035, D-020, D-030 | superseded | Local search defaults to `Sql`; AI/search opt-ins must be lane-compatible and fail fast on lane-owned conflicts. | P2, P7 |
| D-041 | AI | LLM provider switch | Superseded by D-060. Historical decision retained: `AiServices:Provider = AzureInference \| OpenAICompatible \| FoundryLocal \| None`. | D-035, D-030 | superseded | Local AI defaults to `None`; explicit opt-ins must be lane-compatible. | P2, P5 |
| D-042 | Config | Config and feature-flag provider | Superseded by D-060. Historical decision retained: Azure App Configuration + `Microsoft.FeatureManagement` with local fallback. | D-035 | superseded | Configuration remains provider-neutral; lane contract is shared by all hosting and test surfaces. | P5 |
| D-043 | Security | Data Protection persistence switch | Superseded by D-060. Historical decision retained: `DataProtection:Persistence = AzureBlob \| Redis \| None`. | D-035, D-030 | superseded | Azure uses Blob Data Protection; NonAzure uses Redis Data Protection. | P2 |
...
| D-045 | Data | Postgres pooler mode switch | Superseded by D-060. Historical decision retained: `Database:PostgreSql:PoolerMode = None \| Transaction`. | D-020, D-036 | superseded | PostgreSQL is now lane-owned by NonAzure; pooler behavior remains a provider-specific option. | P2, P6, Bicep |
| D-046 | Messaging | Messaging framework retained | Superseded by D-060. Historical decision retained: own outbox and transport adapter, without MassTransit or SlimMessageBus. | D-026, D-029, D-034 | superseded | Service Bus is Azure's lane-owned broker; RabbitMQ is NonAzure's. | P1 |
| D-047 | Runtime | Runtime profile per host | Superseded by D-060. Historical decision retained: shared runtime profiles across hosts, including embedded Scheduler TickerQ and a short-lived migrator profile. | D-010 | superseded | API, gateway, Scheduler, Blazor, and Uno are common; Functions is Azure-only. | G1, F1 |
| D-048 | Serialization | Source-generated JSON | Superseded by D-060. Historical decision retained: source-generated JSON is registered across runtime and messaging surfaces. | D-047 | superseded | The shared lane contract applies to Aspire, Testcontainers, Bicep, Compose, and all three UIs. | G1 |
| D-049 | Observability | Health probe contract | Superseded by D-060. Historical decision retained: `/healthz/live`, `/healthz/ready`, and aggregate `/healthz`. | none | superseded | Health and acceptance contracts are shared across lanes; expensive full-lane and UI acceptance remains manual. | G2 |
