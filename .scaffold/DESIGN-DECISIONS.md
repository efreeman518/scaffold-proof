...
| D-038 | Data | Read-model provider switch | `ReadModel:Provider = Cosmos \| PostgreSqlJsonb \| MongoDb`; NonAzure defaults to PostgreSQL JSONB and permits explicit MongoDB; legacy `Relational` is a one-release alias for `PostgreSqlJsonb`. The relational arm remains a `TaskView` table in the `taskflow` schema with provider-native JSON body storage, counters patched via one `ExecuteUpdateAsync`, and opaque keyset paging. | D-035, D-004, D-030 | superseded | Reuses the existing Cosmos projection contract instead of forking it while making the PostgreSQL JSONB and MongoDB provider choices explicit. The previous `Relational` value is retained only for compatibility. | P2, P3 |
...
| D-042 | Config | Config and feature-flag provider | Azure App Configuration + `Microsoft.FeatureManagement`, gated on `AppConfig:Endpoint`; sentinel-key refresh, Key Vault references for secrets, tenant `TargetingContext`; local fallback is the `FeatureManagement` section in appsettings | D-035 | confirmed | Dynamic flags need a real flag service; the lane contract keeps configuration provider-neutral. Rejected: env-only switches (no runtime toggle without a redeploy); a self-hosted flag service (new infrastructure for a proof). | P5 |
...
| D-044 | Hosting | Azure surface retained in the Portable lane | Superseded by D-060. Historical decision retained: Azure Key Vault and App Configuration remained available in Portable. | D-035, D-036 | superseded | NonAzure is now explicitly non-Azure for its lane-owned core providers; opt-ins must be lane-compatible. | P6, docs |
...
| D-060 | Hosting | Strict Azure and NonAzure hosting lanes | Canonical `TASKFLOW_LANE = Azure \| NonAzure`; `Portable` is a deprecated input alias for `NonAzure` for one release. Azure owns SQL Server, Service Bus, Azure Blob, Cosmos, Azure Table, and Blob Data Protection. NonAzure owns PostgreSQL, RabbitMQ, S3, relational audit, PostgreSQL JSONB read model by default, and Redis Data Protection; MongoDB is an explicit compatible opt-in. Lane-owned core provider conflicts fail fast. Redis, DatabaseMigrator, API, Gateway, Scheduler with embedded TickerQ, Blazor, React, and Uno are common; Functions is Azure-only. Local search defaults to Sql and AI defaults to None; opt-ins must be lane-compatible. SeaweedFS replaces MinIO. Major/family image tags are centralized. Aspire, Testcontainers, Azure Bicep, Compose, and all three UIs share the lane contract; expensive full-lane/UI acceptance is manual. | D-035..D-049 | confirmed | Makes lane ownership explicit while preserving provider-neutral optional seams and historical decision context. | P2, P6, AppHost, Bicep, Compose, UI |
