...
| D-064 | Runtime | Request timeouts, graceful shutdown, runtime evidence | Api and Gateway apply a default request-timeout policy from `RequestTimeouts:DefaultSeconds` (Gateway budget above the Api budget). ServiceDefaults sets `HostOptions.ShutdownTimeout` from `Hosting:ShutdownTimeoutSeconds` (default 25 s, below the 30 s Container Apps termination grace) and a drain step: on stopping, `/healthz/ready` reports unhealthy for `Hosting:DrainDelaySeconds` (default 5 s) before the server stops accepting work. Every host logs `GCSettings.IsServerGC` and the effective GC memory limit at startup. | D-047, D-049 | confirmed | Bounds abandoned work per hop, lets the orchestrator remove a replica from traffic before in-flight requests are cut, and turns the D-047 runtime profile into observed evidence instead of an assumption. | Api, Gateway, ServiceDefaults |
