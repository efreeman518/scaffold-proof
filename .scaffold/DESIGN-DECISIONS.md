...
flowchart TD
    ...
    D034["D-034: Messaging switch (superseded)"]
    ...
    D060["D-060: Strict Azure and NonAzure hosting lanes"]
    D061["D-061: Lane-specific observability backend"]
    D062["D-062: In-house load runner"]
    D063["D-063: No unsafe-method retry"]
    D064["D-064: Request timeouts and graceful shutdown"]
    D065["D-065: Head trace sampling"]
...
| D-034 | Messaging | Historical messaging provider switch | Superseded by D-060. The application transport port still supports Service Bus and RabbitMQ, but a deployed lane cannot flip brokers independently: Azure always owns Service Bus and NonAzure always owns RabbitMQ. | D-026, D-029 | superseded | Strict lane ownership prevents Azure and NonAzure infrastructure from mixing. RabbitMQ consumers remain hosted in Scheduler; Azure Service Bus consumers remain in Functions. | Phase 3.7, D-060 |
| D-035 | Hosting | Hosting-lane preset | Superseded by D-060. Historical decision retained: `TASKFLOW_LANE = Azure \| Portable` seeded independent switch defaults. | D-030, D-034 | superseded | Replaced by the strict Azure/NonAzure contract; the `Portable` spelling remains only as a one-release input alias. | P2, P6, AppHost |
| D-036 | Hosting | NonAzure compute topology | Docker Compose on one VPS with Caddy as the TLS edge in front of the YARP gateway; hand-written files under `deploy/compose/` | D-035 | confirmed | Matches the target topology without an IaC generator dependency. The NonAzure lane retains this shape; Aspire, Testcontainers, Bicep, Compose, and all UIs share the D-060 lane contract. Rejected: Aspire publish to Compose (new `Aspire.Hosting.Docker` dependency, generated IaC harder to hand-tune); k3s/Swarm kept as a documented multi-node upgrade path. | P6 |
| D-037 | Storage | Object-storage provider switch | `Storage:Provider = AzureBlob \| S3` behind the unchanged `EF.Storage.Contracts.IObjectStorageRepository`; S3 arm is the published `EF.Storage.S3.S3ObjectStorageRepository` (SeaweedFS locally/on the VPS, any S3-compatible provider in production); download URLs are presigned against a public endpoint | D-035, D-030 | confirmed | Keeps the attachment port provider-neutral without a second abstraction. SeaweedFS replaces MinIO for the NonAzure S3-compatible service. Rejected: an S3-to-Blob gateway container (extra moving part, still needs a real switch underneath). | P2, P4 |
| D-038 | Data | Read-model provider switch | `ReadModel:Provider = Cosmos \| PostgreSqlJsonb \| MongoDb`; NonAzure defaults to PostgreSQL JSONB and permits explicit MongoDB; legacy `Relational` is a one-release alias for `PostgreSqlJsonb`. The relational arm remains a `TaskView` table in the `taskflow` schema with provider-native JSON body storage, counters patched via one `ExecuteUpdateAsync`, and opaque keyset paging. | D-035, D-004, D-030 | confirmed | Reuses the existing Cosmos projection contract instead of forking it while making the PostgreSQL JSONB and MongoDB provider choices explicit. This supersedes only the old provider value set; the previous `Relational` value is retained only for compatibility. | P2, P3 |
| D-039 | Data | Audit-sink provider switch | `Audit:Provider = AzureTable \| Relational`; relational arm keyed `(TenantId, RecordedUtc, Id)`, retention via `ExecuteDeleteBatchedAsync`; both arms implement `EF.Audit.Contracts.IAuditLogRepository` | D-035, D-030 | confirmed | Same switch shape as the read model, no new pattern. Azure uses Azure Table and NonAzure uses relational audit. Rejected: Azurite as the NonAzure audit sink (still Azure-shaped, does not prove a non-Azure path). See D-058 for recordedUtc/key derivation and package decisions. | P2, P3 |
| D-040 | Search | Search provider switch | `Search:Provider = AzureAiSearch \| PgVector \| Sql`; `PgVector` requires `Database:Provider = PostgreSql` and fails fast at startup otherwise | D-035, D-020, D-030 | confirmed | Local search defaults to `Sql`; AzureAiSearch is a compatible live Azure opt-in, while PgVector is a compatible NonAzure opt-in. A vector search arm should be loud when its prerequisite provider is missing, not silently degrade. Rejected: silent downgrade to `Sql` prefix search; a dedicated vector database (new infrastructure for a proof). | P2, P7 |
| D-041 | AI | LLM provider switch | `AiServices:Provider = AzureInference \| OpenAICompatible \| None`; local default is `None`; AzureInference is a compatible live Azure opt-in, and OpenAICompatible is the compatible NonAzure opt-in | D-035, D-030 | confirmed | One SDK covers every OpenAI-compatible vendor rather than one integration per vendor. Rejected: per-vendor SDKs (N integrations for the same wire protocol) and native local-model runtimes with fragile lifecycle management. | P2, P5 |
| D-042 | Config | Config and feature-flag provider | Azure App Configuration + `Microsoft.FeatureManagement` is an Azure live opt-in, gated on `AppConfig:Endpoint`; NonAzure rejects non-empty Azure App Configuration configuration and uses appsettings/environment. Sentinel-key refresh, Key Vault references, and tenant `TargetingContext` apply only when the Azure opt-in is selected. | D-035 | confirmed | Dynamic flags need a real flag service in Azure; zero-Azure NonAzure must remain deployable without Azure configuration. Rejected: silently accepting Azure endpoints in NonAzure; a self-hosted flag service (new infrastructure for a proof). | P5 |
| D-043 | Security | Data Protection persistence switch | `DataProtection:Persistence = AzureBlob \| Redis \| None`; Azure uses Blob Data Protection and NonAzure uses Redis Data Protection; `None` logs a warning because payloads break across replicas | D-035, D-030 | confirmed | Persistence and protection are separable; only persistence needs a NonAzure arm. Rejected: a file-share ring (single point of failure, no multi-VPS story); an ephemeral ring as the default (silently breaks multi-replica cursors). | P2 |
| D-044 | Hosting | Azure surface retained in the Portable lane | Superseded by D-060. Historical decision retained: Azure Key Vault and App Configuration remained available in Portable. | D-035, D-036 | superseded | NonAzure is now a zero-Azure lane: non-empty Azure App Configuration and Key Vault service configuration is rejected. Azure App Configuration and Key Vault remain Azure live opt-ins; NonAzure uses appsettings/environment and the existing local encryption-key path. | P6, docs |
| D-045 | Data | Postgres pooler mode switch | `Database:PostgreSql:PoolerMode = None \| Transaction`; `Transaction` appends `No Reset On Close=true;Max Auto Prepare=0` to the Npgsql connection string; NonAzure runs a PgBouncer container | D-020, D-036 | confirmed | Transaction-mode pooling is the throughput-relevant PgBouncer mode and needs explicit connection-string cooperation from Npgsql. Rejected: session-mode-only pooling (does not multiplex connections, defeats the purpose at scale). | P2, P6, Bicep |
| D-046 | Messaging | Messaging framework retained | Own outbox + `IIntegrationEventTransport` + `EF.Messaging.RabbitMq` stays the transport layer; MassTransit and SlimMessageBus are not adopted. Service Bus is Azure's broker; RabbitMQ is NonAzure's. | D-026, D-029, D-034 | confirmed | The outbox and consumer host already exist and work on both brokers; a framework would duplicate both. Rejected: MassTransit and SlimMessageBus (would duplicate the existing outbox and consumer host). | P1 |
| D-047 | Runtime | Runtime profile per host | Shared runtime profiles remain in force for API, Gateway, Scheduler with embedded TickerQ, Blazor, Uno, and Azure-only Functions; DatabaseMigrator keeps its short-lived workstation profile | D-010 | confirmed | Matches each host's actual workload shape instead of one blanket GC setting. Native AOT remains deferred because EF Core, TickerQ, and FlowEngine are reflection-based. | G1, F1 |
| D-048 | Serialization | Source-generated JSON | One `TaskFlowJsonContext : JsonSerializerContext` remains registered across runtime, messaging, cache, and UI resolver chains; reflection stays after it for third-party types. | D-047 | confirmed | Source generation is the low-risk, high-value half of the AOT story even where full AOT is deferred. Queue payload stays JSON; binary serialization remains scoped to the L2 cache value. | G1 |
| D-049 | Observability | Health probe contract | `/healthz/live` (self only), `/healthz/ready` (database, outbox, scheduler, broker on consumer hosts; cache excluded because it degrades to L1), `/healthz` aggregate for humans and Compose healthchecks; `/readyz` removed | none | confirmed | Liveness must never fail on a dependency a restart cannot fix; readiness is where dependency checks belong. The probes are shared across Azure and NonAzure lanes. Rejected: a single combined probe; keeping `/readyz` alongside `/healthz/ready`. | G2 |
| D-060 | Hosting | Strict Azure and NonAzure hosting lanes | Canonical `TASKFLOW_LANE = Azure \| NonAzure`; `Portable` is a deprecated input alias for `NonAzure` for one release. Azure owns SQL Server, Service Bus, Azure Blob, Cosmos, Azure Table, and Blob Data Protection. NonAzure owns PostgreSQL, RabbitMQ, S3, relational audit, PostgreSQL JSONB read model by default, and Redis Data Protection; MongoDB is an explicit compatible opt-in. Lane-owned core provider conflicts fail fast. NonAzure is zero-Azure: non-empty Azure App Configuration and Key Vault service configuration is rejected; it uses appsettings/environment and the existing local encryption-key path. Azure App Configuration and Key Vault remain Azure live opt-ins. Redis, DatabaseMigrator, API, Gateway, Scheduler with embedded TickerQ, Blazor, React, and Uno are common; Functions is Azure-only. Local search defaults to Sql and AI defaults to None; opt-ins must be lane-compatible. SeaweedFS replaces MinIO. Major/family image tags are centralized. Aspire, Testcontainers, Azure Bicep, Compose, and all three UIs share the lane contract; expensive full-lane/UI acceptance is manual. | D-035..D-049 | confirmed | Makes lane ownership explicit while preserving confirmed provider, runtime, serialization, and health decisions; D-034, D-035, D-044, and the old D-038 value set are superseded. | P2, P6, AppHost, Bicep, Compose, UI |
| D-061 | Observability | Lane-specific observability backend | Local development exports OpenTelemetry to the Aspire Dashboard only. Deployed NonAzure runs pinned OpenObserve OSS v1.0.0 in its own persistent container for OTLP logs and traces; metrics export is disabled by default, retention is short and operator-configurable, and the UI binds to a private/loopback interface by default. All ProblemDetails responses expose the server request identifier as `requestId` and W3C `traceId`/`spanId` fields that correlate exceptions, logs, and traces across HTTP and broker hops. Applications export directly through the OpenTelemetry SDK; an OpenTelemetry Collector is a later opt-in only if tail sampling becomes necessary. | D-053, D-060 | confirmed | This is the minimum self-hosted diagnostic surface that supports exception investigation and cross-service request tracing without ingesting low-value metrics. OpenObserve Open Source Edition is AGPL-3.0; hosted and enterprise offerings have separate terms. The selected single-node deployment is a low-license-cost baseline, not high availability. Rejected: deployed `otel-lgtm` because that image is a development/demo stack; always-on metrics; adding a Collector before a concrete sampling need exists. | Compose, ServiceDefaults, exception handling, operations docs |
...
| D-062 | Testing | In-house load runner replaces NBomber | `tests/Test.Load/LoadRunner.cs`: open-model fixed-arrival-rate runner (latency measured from the scheduled start, requests past `maxInFlight` dropped and counted as errors) with nearest-rank p50/p95/p99. Scenarios assert error rate and p95/p99 budgets and stay manual against a hosted stack. A non-Load-category percentile check keeps the runner itself under the normal test run. | - | confirmed | NBomber requires a paid Business license for organizational use; scaffold-ai GR-04 excludes paid licenses when a free option exists, and a ~70-line runner covers the need. Removing it also retires the NBomber transitive-advisory exception (the MessagePack pin stays for D-048). | Test.Load, Directory.Packages.props |
| D-063 | Resilience | No retry of unsafe HTTP methods | ServiceDefaults and the Blazor Refit clients register the standard handler with `Retry.DisableForUnsafeHttpMethods()`, so POST/PATCH/PUT/DELETE/CONNECT are sent once. The Blazor gRPC read client keeps retries: every gRPC call is an HTTP POST, and that client serves only idempotent reads. D-051 read hedging stays nested inside the standard handler, GET-only. | D-051, D-054 | confirmed | The standard handler retries every method by default, which duplicated creates and uploads on a 5xx or timeout. | ServiceDefaults, Blazor |
| D-064 | Runtime | Request timeouts, graceful shutdown, runtime evidence | Api and Gateway apply a default request-timeout policy from `RequestTimeouts:DefaultSeconds` (Gateway budget above the Api budget). ServiceDefaults sets `HostOptions.ShutdownTimeout` from `Hosting:ShutdownTimeoutSeconds` (default 25 s, below the 30 s Container Apps termination grace) and a drain step: on stopping, `/healthz/ready` reports unhealthy for `Hosting:DrainDelaySeconds` before the server stops accepting work. The drain defaults to 0 (tests and local runs stop immediately); the Container Apps serving apps set 5 s, and the migrator job sets none. Compose sets `Hosting__ShutdownTimeoutSeconds=8` to stay under its 10 s default stop grace and needs no drain because nothing routes by readiness there. Every host logs `GCSettings.IsServerGC` and the effective GC memory limit at startup. | D-047, D-049 | confirmed | Bounds abandoned work per hop, lets the orchestrator remove a replica from traffic before in-flight requests are cut, and turns the D-047 runtime profile into observed evidence instead of an assumption. | Api, Gateway, ServiceDefaults |
| D-065 | Observability | Head trace sampling | `OpenTelemetry:Tracing:SampleRatio` (0.0-1.0). OTLP export uses `ParentBased(TraceIdRatioBased(ratio))`, default 1.0. The Azure Monitor distro keeps its own default rate-limited sampling unless the ratio is set, in which case `SamplingRatio` is applied and `TracesPerSecond` cleared (it takes precedence when both are set). Tail sampling remains the D-061 Collector opt-in. | D-061 | confirmed | Always-on tracing at high request rates costs ingestion without adding diagnostic value; the ratio is an environment knob, not a code change. | ServiceDefaults |
