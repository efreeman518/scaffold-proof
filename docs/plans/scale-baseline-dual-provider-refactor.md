...
| S2 | Phase 1.1-1.4 + 1.6 data layer: provider switch, model (composite PK, Version, UTC, encryption, inbox/outbox/work entities + configs), 6 migration sets, migrator, validator, AppHost, test fixtures both providers | fable | running | feature/sb-s2-data-layer | ../scaffold-proof-wt/s2 | a90c2662277d7a838 |
...
| S6 | Phase 3.1-3.2 + 3.7 messaging: outbox staging interceptor + domain events, claim/dispatch workers, transport port with ServiceBus AND RabbitMq implementations (Messaging:Provider switch, connection multiplexer, publisher confirms, per-queue prefetch, DLX, consumer hosted services in Scheduler), envelope, Functions split + DLQ + disabled-when-RabbitMq, consumer inbox guard, projection patch/continuation, Aspire SB subscriptions or RabbitMQ resource, rabbitmq-container-app.bicep, artifacts for D-034 | opus | blocked on S2 | feature/sb-s6-messaging | ../scaffold-proof-wt/s6 | - |
