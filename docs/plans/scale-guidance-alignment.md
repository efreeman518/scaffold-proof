...
| 7 | Server GC + DATAS | Done | `src/Host/TaskFlow.Host.props:23-58` (`ServerGarbageCollection`, `ConcurrentGarbageCollection`, `GarbageCollectionAdaptationMode=1`, `TieredPGO`, `InvariantGlobalization`) | Imported explicitly by the six host csproj files (Api, Gateway, Scheduler, Blazor, Functions, DatabaseMigrator); DatabaseMigrator overrides `ServerGarbageCollection` to `false` for its short-lived-job profile. `InvariantGlobalization` defaults to `true` (plain `-chiseled` base) but Api, Scheduler, DatabaseMigrator, and Functions override it to `false` on the `-chiseled-extra` (ICU) base: their dependency closure includes `Microsoft.Data.SqlClient` (via `TaskFlow.Infrastructure.Data`, directly or through `TaskFlow.Bootstrapper`), which throws `NotSupportedException` under invariant globalization (F1); Blazor overrides it for the same base-image reason plus rendering user-facing cultures. Only Gateway keeps the default. Verified live (F1): `dotnet run --no-build` for the Api against a real SQL Server LocalDB opens a `Microsoft.Data.SqlClient` connection and Kestrel binds both configured ports with no error, on the unmodified build output. `HostRuntimeSettingsTests` at `tests/Test.Architecture/HostRuntimeSettingsTests.cs` enforces the import, every override, the SqlClient-dependent hosts never declaring `InvariantGlobalization=true`, and the ICU-image pairing. |
...
- Newly found and closed in the same pass: `Microsoft.Data.SqlClient` throws `NotSupportedException` under
  invariant globalization, and Api/Scheduler/DatabaseMigrator/Functions all inherited
  `InvariantGlobalization=true` from D-047's shared profile - a live `dotnet run` against a real SQL Server
  never worked, only Testcontainers-based tests masked it (they don't import `TaskFlow.Host.props`). Fixed
  by overriding `InvariantGlobalization=false` on those four hosts and moving their Dockerfiles to
  `-chiseled-extra`; see row 7 above and D-047 in DESIGN-DECISIONS.md.
