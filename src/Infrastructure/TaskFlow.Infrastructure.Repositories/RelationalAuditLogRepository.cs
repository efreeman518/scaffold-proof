    ...
    : RepositoryBase<TaskFlowDbContextTrxn, string, Guid?>(db), IAuditLogRepository
{
        ...
        AuditEntry<TAuditIdType, TTenantIdType> entry, CancellationToken cancellationToken = default)
    {
        ...
        return UpsertAsync(record, e => new { e.TenantId, e.RecordedUtc, e.Id }, cancellationToken: cancellationToken);
    }
    ...
    public Task<int> PurgeOlderThanAsync(DateTimeOffset cutoffUtc, CancellationToken cancellationToken = default) =>
        ...
        DB.AuditLog.ExecuteDeleteBatchedAsync(
            e => e.RecordedUtc < cutoffUtc, e => e.Id, purgeBatchSize, ct: cancellationToken);
